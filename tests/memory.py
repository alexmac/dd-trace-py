...
class KitchenSink(object):
    ...
    def __init__(self):
        ...
        self._pymongo = pymongo.MongoClient(**config.MONGO_CONFIG)
        self._pylibmc = pylibmc.Client(["%s:%s" % (
            config.MEMCACHED_CONFIG["host"],
            config.MEMCACHED_CONFIG["port"])])


    def ping(self, i):
        with ddtrace.tracer.trace("kitchensink", service="kitchensink"):
            self._ping_redis(i)
            self._ping_pg(i)
            self._ping_pymongo(i)
    ...
    def _ping_pymongo(self, i):
        db = self._pymongo["test-db"]
